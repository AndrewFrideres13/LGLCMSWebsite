...
(function (factory) {
  ...
  $.colorpicker = new (function () {
    ...
    this.parsers = {
      ...
      RGBA: function (color) {
        var m = /^rgba?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color(
      ...
      "RGBA%": function (color) {
        var m = /^rgba?\(\s*(\d+(?:\.\d+)?)\%\s*,\s*(\d+(?:\.\d+)?)\%\s*,\s*(\d+(?:\.\d+)?)\%\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color(
      ...
      HSLA: function (color) {
        var m = /^hsla?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color()
      ...
      "HSLA%": function (color) {
        var m = /^hsla?\(\s*(\d+(?:\.\d+)?)\%\s*,\s*(\d+(?:\.\d+)?)\%\s*,\s*(\d+(?:\.\d+)?)\%\s*(?:,\s*(\d+(?:\.\d+)?)\s*)?\)$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color()
      ...
      "#HEXA": function (color) {
        var m = /^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color(
      ...
      "#HEXA4": function (color) {
        var m = /^#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])$/.exec(
          color
        );
        if (m) {
          return new $.colorpicker.Color(
    ...
    this.parts = {
      header: function (inst) {
        ...
        this.disable = function (disable) {
          $(".ui-dialog-titlebar-close", part)[disable ? "off" : "on"](
            "click",
            _onclick
          );
        };
      ...
      map: function (inst) {
        ...
        _keydown = function (event) {
          ...
          if (typeof x_change[event.which] !== "undefined") {
            ...
            inst.color.setChannel(
              x_channel_map[inst.mode],
              (value + change) / width
            );
            inst._change(false);
        ...
        _html = function () {
          var html =
            ...
            ' ui-colorpicker-border" taonex="0">' +
      ...
      preview: function (inst) {
        ...
        this.disable = function (disable) {
          $(".ui-colorpicker-preview-initial", part)[disable ? "off" : "on"](
            "click",
            onclick
          );
        };
      ...
      rgb: function (inst) {
        ...
        this.init = function () {
          ...
          $(".ui-colorpicker-number", part).on(
            ...
            function () {
              var r = $(
                ".ui-colorpicker-rgb-r .ui-colorpicker-number",
                part
              ).val();
              inst.color.setRGB(
                inputs.r.val() / 255,
      ...
      hex: function (inst) {
        var that = this,
          ...
          html = function () {
            ...
            return (
              '<div class="ui-colorpicker-hex"><label>#</label>' +
              html +
              "</div>"
            );
          };
        ...
        this.init = function () {
          ...
          inputs.color.on("keydown keyup", function (e) {
            return (
              e.ctrlKey ||
              e.metaKey ||
              _keycode.isHex(e.which) ||
              !_keycode.isPrint(e.which)
            );
          });
          ...
          inputs.color.on("change keyup", function () {
            ...
            inst.color = parseHex(inputs.color.val()).setAlpha(
              inst.color.getAlpha()
            );
            inst._change();
          });
      ...
      swatches: function (inst) {
        var that = this,
          ...
          onclick = function () {
            inst.color =
              inst._parseColor($(this).css("background-color")) ||
        ...
        this.disable = function (disable) {
          $(".ui-colorpicker-swatch", part)[disable ? "off" : "on"](
            "click",
            onclick
          );
        };
      ...
      footer: function (inst) {
        var that = this,
          part = null,
          id_transparent =
            "ui-colorpicker-special-transparent-" + inst.colorpicker_index,
        ...
        function rgbToHSLConverter(color) {
        ...
        this.init = function () {
          ...
          //Okay Button pressed when the button is active AND clickable
          $(".ui-colorpicker-ok", part)
            .button()
            .click(function () {
              //Get the value of the text inside the input field
              var textColorToPass = $("#my_color_picker").val();
              var triadh2 = rgbToHSLConverter(textColorToPass)[0];
              triadh2 += 120;
              var triads2 = rgbToHSLConverter(textColorToPass)[1];
              var triadl2 = rgbToHSLConverter(textColorToPass)[2];
              var triad2 =
                "hsl(" + triadh2 + 120 + "," + triads2 + "%," + triadl2 + "%);";
              var invertedColor = triad2;
              var triadh3 = rgbToHSLConverter(textColorToPass)[0];
              var triads3 = rgbToHSLConverter(textColorToPass)[1];
              var triadl3 = rgbToHSLConverter(textColorToPass)[2];
              triadh3 += 240;
              var triadColor3 =
                "hsl(" + triadh3 + "," + triads3 + "%," + triadl3 + "%);";

              //Prepare the string to send & is the seperator of variables with post UNLESS we pass an obj
              var dataToSend =
                "textcolor=" +
                textColorToPass +
                "&inverted=" +
                ...
                "&triad3=" +
                triadColor3;
              //Attempt to scrub any invalid chars before we pass it along
              let invalid_characters = ["$", "#", "%", "<", ">", "|"];
              dataToSend = dataToSend.replace(invalid_characters, "");
              //Begin Ajax Call
              $.ajax({
                type: "POST", //Response type (POST,GET)
                url: "colorpicker.php", //Path to php file to communicate with
                cache: false, //Cache for future use? Is this cachable?
                data: dataToSend, //Data to send via our event type
                //Callback function to perform if sucessful
                success: function (result) {
                  //Appends the html and result which actually makes the color change
                  $(".debug").html(result);
                },
                //Error callback function, which will also be appended to the resul
                error: function (result) {
                  console.log(result); //Will log whatever happens
                },
              }); //End Ajax
              inst.close();
            });
          ...
          $(".ui-colorpicker-buttonset", part)[
            $.fn.controlgroup ? "controlgroup" : "buttonset"
          ]();
        ...
        this.repaint = function () {
          $(".ui-colorpicker-special-none", part)
            .prop("checked", !inst.color.set)
            .button("refresh");
          $(".ui-colorpicker-special-transparent", part)
            .prop("checked", inst.color.set && inst.color.getAlpha() === 0)
            .button("refresh");
          $(".ui-colorpicker-ok", part).button(
  ...
  $.widget("vanderlee.colorpicker", {
        ...
        if (/\bbutton|all|both\b/.test(that.options.showOn)) {
          ...
          that.button.insertAfter(that.element).click(function () {
            if (!that.options.disabled) {
              if (
                that.opened &&
                /\bbutton|all|both\b/.test(that.options.hideOn)
              ) {
                that.close();
              } else {
                that.open();
    ...
    _generatePopup: function () {
      ...
      if (that.events.document_click_html === null) {
        $(document).on(
          "touchstart click",
          "html",
          (that.events.document_click_html = function (event) {
            if (
              !that.opened ||
            ...
            for (p = 0; p <= parents.length; ++p) {
              ...
              // showOn alt
              if (
                /\balt|all|both\b/.test(that.options.showOn) &&
      ...
      if (that.events.document_keydown === null) {
        $(document).on(
          "keydown",
          (that.events.document_keydown = function (event) {
            // close on ESC key
            if (
              that.opened &&
              event.keyCode === 27 &&
    ...
    _effectGeneric: function (element, show, slide, fade, callback) {
      ...
      if ($.effects && $.effects[that.options.showAnim]) {
        ...
        if (
          $.isFunction(callback) &&
          (!that.options.showAnim || !that.options.duration)
        ) {
          callback();
        }
      }
    ...
    open: function () {
      ...
      if (!that.opened) {
        ...
        if (that.options.modal) {
          that.overlay = $('<div class="ui-widget-overlay"></div>')
            .appendTo("body")
    ...
    destroy: function () {
      ...
      if (that.events.document_click_html !== null) {
        $(document).off(
          "touchstart click",
          "html",
          that.events.document_click_html
        );
      }
    ...
    _change: function (stoppedChanging /* = true */) {
      ...
      if (
        this.color.set &&
        this.options.limit &&
        $.colorpicker.limits[this.options.limit]
      ) {
      ...
      if (!this.inline) {
        if (!this.color.set) {
          ...
          this.element
            .val(this._formatColor(this.options.colorFormat, this.color))
            .change();
        }
    ...
    _parseFormat: function (format, text) {
      ...
      pattern = pattern.replace(/\\?[argbhsvcmykLAB][xdfp]/g, function (
        variable
      ) {
        if (variable.match(/^\\/)) {
    ...
    _closestName: function (color) {
      ...
      this._eachSwatch(function (n, swatch) {
        ...
        if (d < distance || distance === null) {
          name = n;
          if (d <= 1e-20) {
            // effectively 0 by maximum rounding error
...
//Fire up the Color Selector right away, and have it detect the
//element with my_color_picker
